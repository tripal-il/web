...
const BusTimes: React.FC<{
  data: TripInformation;
  reversed: boolean;
  lon: number;
  lat: number;
}> = ({ data, reversed, lat, lon }) => {
  ...
  let closestStop: {
    ...
    stop_lon: number;
    stop_lat: number;
  } = { stop_id: "", stop_name: "", stop_lon: 0, stop_lat: 0 };
        ...
        return (
          <div>
            <h4 className="title is-5">
              {stop.stop_name} ({closestDistance.toFixed(2)}km away,{" "}
              {walkingTime(closestDistance)})
            </h4>
          </div>
        );
      } else {
      ...
      <div className="card">
        <div className="card-content">
          ...
          <div className="media-content">
            ...
            <BusTimes
              data={trip_info}
              reversed={isReversed as boolean}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  for (const stop of stop_times) {
    const foundStop = stps.find(
      (foundStop) => foundStop.stop_id === stop.stop_id,
    ) as Stop;
    data.push({
      stop_id: stop.stop_id,
      stop_name: foundStop.stop_name,
      stop_sequence: stop.stop_sequence,
      stop_lon: foundStop.stop_lon,
      stop_lat: foundStop.stop_lat,
    });
  }
